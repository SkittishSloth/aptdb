...
interface AptSourceProvider {
  fun sources(aptConfig: AptConfiguration): List<AptSource>? =
    aptConfig.sourceList
             ?.readLines()
             ?.map { AptSource.from(it) }
}

object DefaultAptSourceProvider : AptSourceProvider { }
...
data class AptSource private constructor(
  ...
  companion object {
    fun from(line: String): AptSource {
