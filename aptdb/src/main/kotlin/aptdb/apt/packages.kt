...
interface AptPackageProvider {
  fun packages(packageFile: Path): List<AptPackage> =
    packageFile.readLines()
               .windowedBy { it == "" }
               .map { AptPackage.from(it) }
}

object DefaultAptPackageProvider : AptPackageProvider { }
...
data class AptPackage(
  ...
  companion object {
    fun from(lines: List<String>): AptPackage =
